...
Réponds UNIQUEMENT avec le JSON valide, sans backticks.`;
  ...
  function exportPDF() {
    ...
    const rawHtml = rawFields.map(([lbl, val]) => `
      <div style="margin-bottom:14px">
        <div style="font-size:8px;font-weight:700;color:#999;text-transform:uppercase;margin-bottom:4px">${lbl}</div>
        <div style="font-size:10px;color:#111;line-height:1.6">${val}</div>
      </div>`).join("");

    const aiHtml = aiFields.map(([lbl, val]) => `
      <div style="margin-bottom:18px;padding-bottom:18px;border-bottom:1px solid #eee">
        <div style="font-size:8px;font-weight:700;color:#999;text-transform:uppercase;margin-bottom:5px">${lbl}</div>
    ...
    const filename = `Brief_${brand.replace(/\s+/g,"_")}.html`;
